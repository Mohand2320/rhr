...
type RestOf<T extends IExamen | NewExamen> = Omit<T, 'datePrevue' | 'dateExamen'> & {
...
export class ExamenService {
  ...
  protected convertDateFromClient<T extends IExamen | NewExamen | PartialUpdateExamen>(examen: T): RestOf<T> {
    return {
      ...examen,
      datePrevue: examen.datePrevue?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restExamen: RestExamen): IExamen {
    return {
      ...restExamen,
      datePrevue: restExamen.datePrevue ? dayjs(restExamen.datePrevue) : undefined,
