...
type RestOf<T extends IDemande | NewDemande> = Omit<T, 'dateDepot'> & {
...
export class DemandeService {
  ...
  protected convertDateFromClient<T extends IDemande | NewDemande | PartialUpdateDemande>(demande: T): RestOf<T> {
    return {
      ...demande,
      dateDepot: demande.dateDepot?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDemande: RestDemande): IDemande {
    return {
      ...restDemande,
      dateDepot: restDemande.dateDepot ? dayjs(restDemande.dateDepot) : undefined,
    };
  }
