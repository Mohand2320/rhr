...
import { EntityArrayResponseType, AgenceService } from '../service/agence.service';
...
export class AgenceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAgence): number => this.agenceService.getAgenceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.agences = this.refineData(dataFromBody);
  }

  protected refineData(data: IAgence[]): IAgence[] {
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.agenceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
